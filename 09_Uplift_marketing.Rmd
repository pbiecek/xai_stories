...
* uplift models thanks to its additivity give wide range of possibilities while using XAI  
...
As we can see (Table \@ref(tab:overfitTable)) for the overfitted model, while the overfit gap in accuracy scores is only around 2%, cumulative gain drops by 97%. 
...
Experimental results proved that these two ways of calculating SHAP values are providing close estimations with precision to numerical errors. There are few features that depending on the method have a small positive or negative value. This is caused by the fact that for the estimation of SHAP values directly using uplift model the KernelExplainer was used. It has data parameter however in case of big dataset it is recommended in the documentation to take subset of records. In our case we took random subset therefore this could impact the results. Nevertheless, we proved that in the case of our example the two methods lead to similar values.
